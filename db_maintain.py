...
def build_tickdb(stock_list):
    ...
    for s_code in stock_list:
        count += 1
        print('Start processing {}...{{{:>4d}:{}}}'.format(s_code,count,stock_count))
        #first check wheather indexed
        idxinfo = db[s_code].index_information()
        if not (len(idxinfo) == 2 and 'date_1' in idxinfo.keys()):
            print("No index info found for {}:{}".format(s_code, idxinfo.keys()))
        ...
        if not k_data.empty:
            ...
            for i in range(1,len(k_data)+1):
                ...
                if compdays(daily_record.date) and try_count < 40:
                    if not db[s_code].find_one({'date':daily_record.date}, {'_id':0}):
                        db[s_code].insert_one(json.loads(daily_record.to_json()))
                        print('{:13s} {}...{{{:>4d}:{}}}'.format('insert K data '+s_code, daily_record.date ,count,stock_count))
                    else:
                        try_count += 1
                else:
                    break
        ...
        for post_d in db[s_code].find({'tick' : {"$exists":False}}):
            ...
            try:
                if not tk.empty:
                    ...
                    print('{:13s} {}...{{{:>4d}:{}}}'.format('processing ...' + s_code, day,count,stock_count))    
            except AttributeError:
                print('skipping, {}, no tick-data got on {}'.format(s_code, day))            
