...
parser.add_argument(
    '-d', '--day', default=date.today().isoformat(), help='target day')
...
class GEN_datasets():
    def __init__(self,
                 split_name,
                 sday=LATEST_STOCK_BASIC_DB_REC_DAY,
                 eday='2016-01-01',
                 data_dir='stock_tfexample',
                 th_size=30,
                 rief=10000):
        self.fname = "tick" + sday.replace(
            '-', "") + "_%s_{:0>2d}.tfrecord" % (split_name)
        ...
        self._records_gzip = tf.python_io.TFRecordOptions(
    ...
    def build_tfexample(self, code, s_record, two_day_later_high):
        ...
        if len(tick) >= self.tk_size:
            ...
            if two_day_later_high:
                feature_Y = self._type_y(
                    (two_day_later_high - s_record.close) / float(
                        s_record.close))
            else:
                feature_Y = 888
            ...
            # of shape (299,299,3)
            return feature_Y, tf.train.Example(
    ...
    def _dealwith_tick(self, code, info_tail, gap=2):
        ...
        for i in range(len(daylist)):
            ...
            if rec_tarday and rec_nexday:
                try:
                    tY, record_example = self.build_tfexample(
                        code, rec_tarday, rec_nexday.high)
                except TypeError:
                    continue

                if record_example:
                    logging.info(
                        self.record_write_str.format(
                            self.count + 1, code, rec_tarday.date,
                            rec_nexday.date, gap, tY, info_tail))
    ...
    def gen_predict_data(self, tar_day, clist=None, gap=2):
        ...
        writer = tf.python_io.TFRecordWriter(
            record_file_name, options=self._records_gzip)
        if clist:
            code_list = clist
        else:
        ...
        for idx, c in enumerate(code_list, start=1):
            logging.info("{} comes, processing ...<{:>4d}:{}>".format(
                c, idx, clist_length))
            ...
            if record:
                _, record_example = self.build_tfexample(
                    c, record, two_day_later_high)
            else:
            ...
            if record_example:
                ...
                logging.info(
                    "{} skipping, None TFrecord formed. Tick length {}".format(
                        c, record.tick.count('datetime')))
    ...
    def gen_stock_tfrecord(self, clist=None, gap=2):
        ...
        if not has_labels(self.data_dir):
            labels_to_class_names = dict(
                zip(range(len(_CLASS_NAMES)), _CLASS_NAMES))
        ...
        for idx, c in enumerate(code_list, start=1255):
            ...
            record_file_t0 = os.path.join(
                self.data_dir, "tick" + LATEST_STOCK_BASIC_DB_REC_DAY.replace(
                    '-', "") + "_t0_%s.tfrecord" % c)
            record_file_t1 = os.path.join(
                self.data_dir, "tick" + LATEST_STOCK_BASIC_DB_REC_DAY.replace(
                    '-', "") + "_t1_%s.tfrecord" % c)
            record_file_t2 = os.path.join(
                self.data_dir, "tick" + LATEST_STOCK_BASIC_DB_REC_DAY.replace(
                    '-', "") + "_t2_%s.tfrecord" % c)
            #        writer = tf.python_io.TFRecordWriter(record_file_name, options=self._records_gzip)
            writer0 = tf.python_io.TFRecordWriter(
                record_file_t0, options=self._records_gzip)
            writer1 = tf.python_io.TFRecordWriter(
                record_file_t1, options=self._records_gzip)
            writer2 = tf.python_io.TFRecordWriter(
                record_file_t2, options=self._records_gzip)
            if self._test_sample_capity(c):
                #                self._dealwith_tick(c, writer, "{:4d}|{}".format(idx, len(code_list)))
                for y, rec in self._dealwith_tick(
                        c, "{:4d}|{}".format(idx, len(code_list)), gap=gap):
                    if y == 0:
...
#            writer2.close()
        logging.info(
            "Totally Type0 counts:{}, Type1 counts:{}, Type2 counts:{}".format(
                self.countt0, self.countt1, self.countt2))


def read_stock_tfrecord(fname):
    rec_opt = tf.python_io.TFRecordOptions(
        tf.python_io.TFRecordCompressionType.GZIP)
...
def main():
    ...
    lv_se_shi_ping = [
        '000672', '002194', '002460', '300078', '300304', '300458', '300466',
        '300496', '600139', '600392', '601988', '603616', '603799', '603822'
    ]
